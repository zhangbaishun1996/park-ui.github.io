...
function atxt(){
	var a_val = $.makeArray($("#plateipDID ul li a")).filter(function(ele,idx,arr){return arr.length - 1 !== idx});
	a_val.forEach(function(item,index){
		if(item.innerText == ''||item.innerText == null){
			$('#cxwdcl').attr('disabled','disabled');
			$('#cxwdcl').addClass('def').removeClass('on');
		}else{
			$('#cxwdcl').attr('disabled',false);
			$('#cxwdcl').addClass('on').removeClass('def');
		}
	})
}
//获取车牌号
